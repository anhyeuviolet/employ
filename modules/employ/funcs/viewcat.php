...
$base_url = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '/' . $global_array_cat[$catid]['alias'];
...
if (empty($contents)) {
    ...
    if ($viewcat == 'viewcat_page_new' or $viewcat == 'viewcat_page_old') {
		...
		$db_slave->sqlreset()
			...
			->where('status= 1 AND inhome=1 AND catid=' . $catid);
		...
		$contents = ThemeEmployCat($viewcat, $array_catpage, $generate_page);
