...
$db_slave->sqlreset()
    ->select('id, catid, addtime, title, alias, about_family, homeimgthumb, homeimgfile')
    ->order('addtime DESC')
    ->limit(30);
...
if ($module_info['rss']) {
    ...
    while (list($id, $catid_i, $addtime, $title, $alias, $about_family, $homeimgthumb, $homeimgfile) = $result->fetch(3)) {
        ...
        $items[] = array(
            ...
            'description' => $rimages . $about_family,
            'pubdate' => $addtime
        );
    }
}
