...
if (!nv_function_exists('nv_employ_category')) {
    function nv_block_config_employ_category($module, $data_block, $lang_block)
    {
    ...
    function nv_block_config_employ_category_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array( );
        $return['error'] = array( );
    ...
    function nv_employ_category($block_config)
    {
            ...
            foreach ($module_array_cat as $cat) {
                if ($block_config['catid'] == 0 and $cat['parentid'] == 0 or ($block_config['catid'] > 0 and $cat['parentid'] == $block_config['catid'])) {
                    ...
                    if (!empty($cat['subcatid'])) {
                        $xtpl->assign('SUBCAT', nv_employ_sub_category($cat['subcatid'], $title_length, $block_theme));
    ...
    function nv_employ_sub_category($list_sub, $title_length, $block_theme)
    {
        ...
        if (empty($list_sub)) {
            ...
            foreach ($list as $catid) {
                ...
                if (!empty($subcat['subcatid'])) {
                    $xtpl->assign('SUB', nv_employ_sub_category($subcat['subcatid'], $title_length, $block_theme));
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_employ_category($block_config);
    }
}
