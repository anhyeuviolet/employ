...
$error = $admins = '';
$savecat = 0;
list ($catid, $parentid, $title, $titlesite, $alias, $description, $descriptionhtml, $groups_view, $viewdescription) = array(
    0,
    0,
    '',
    '',
...
if (! empty($savecat)) {
    ...
    if ($catid == 0 and $title != '') {
        ...
        $stmt = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_cat SET parentid= :parentid, title= :title, titlesite=:titlesite, alias = :alias, description = :description, descriptionhtml = :descriptionhtml, viewdescription= :viewdescription, groups_view= :groups_view, edit_time=' . NV_CURRENTTIME . ' WHERE catid =' . $catid);
...
foreach ($global_array_cat as $catid_i => $array_value) {
    ...
    if (defined('NV_IS_MODADMIN') or (isset($array_cat_admin[$admin_id][$catid_i]) and $array_cat_admin[$admin_id][$catid_i]['admin'] == 1)) {
        $xtitle_i = '';
        if ($lev_i > 0) {
            $xtitle_i .= '&nbsp;&nbsp;&nbsp;|';
            for ($i = 1; $i <= $lev_i; ++ $i) {
...
$xtpl = new XTemplate('job.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
